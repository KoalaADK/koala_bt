...
public class KoalaCommands {
	...
	public static String prepareSpeedCommandInt(int speed_motor_left, int speed_motor_right) {
		if (speed_motor_left > 127) {
			speed_motor_left = 127;
		}
		if (speed_motor_left < -127) {
			speed_motor_left = -127;
		}
		if (speed_motor_right > 127) {
			speed_motor_right = 127;
		}
		if (speed_motor_right < -127) {
			speed_motor_right = -127;
		}
		return prepareSpeedCommand((byte) speed_motor_left, (byte) speed_motor_right);
	}
